package dev.neuronic.net.math.ops.vector;

import dev.neuronic.net.math.Vectorization;
import dev.neuronic.net.math.ops.ElementwiseSqrt;
import jdk.incubator.vector.FloatVector;
import jdk.incubator.vector.VectorSpecies;

/**
 * Vector implementation of ElementwiseSqrt.
 * This class is only loaded when Vector API is available.
 */
public final class ElementwiseSqrtVector implements ElementwiseSqrt.Impl {
    
    @Override
    public void compute(float[] input, float[] output) {
        if (!Vectorization.shouldVectorize(input.length)) {
            scalarCompute(input, output);
            return;
        }
        
        VectorSpecies<Float> species = Vectorization.getSpecies();
        int upperBound = Vectorization.loopBound(input.length);
        
        int i = 0;
        for (; i < upperBound; i += species.length()) {
            FloatVector v = FloatVector.fromArray(species, input, i);
            v.sqrt().intoArray(output, i);
        }
        
        for (; i < input.length; i++) {
            output[i] = (float) Math.sqrt(input[i]);
        }
    }
    
    @Override
    public void computeWithEpsilon(float[] input, float epsilon, float[] output) {
        if (!Vectorization.shouldVectorize(input.length)) {
            scalarComputeWithEpsilon(input, epsilon, output);
            return;
        }
        
        VectorSpecies<Float> species = Vectorization.getSpecies();
        FloatVector epsilonVec = FloatVector.broadcast(species, epsilon);
        int upperBound = Vectorization.loopBound(input.length);
        
        int i = 0;
        for (; i < upperBound; i += species.length()) {
            FloatVector v = FloatVector.fromArray(species, input, i);
            v.add(epsilonVec).sqrt().intoArray(output, i);
        }
        
        for (; i < input.length; i++) {
            output[i] = (float) Math.sqrt(input[i] + epsilon);
        }
    }
    
    private void scalarCompute(float[] input, float[] output) {
        for (int i = 0; i < input.length; i++) {
            output[i] = (float) Math.sqrt(input[i]);
        }
    }
    
    private void scalarComputeWithEpsilon(float[] input, float epsilon, float[] output) {
        for (int i = 0; i < input.length; i++) {
            output[i] = (float) Math.sqrt(input[i] + epsilon);
        }
    }
}